...
const insertedRow = <Change extends RowChangeType & "Inserted", Table extends TableName, Row extends Option.Some>(type: Change, table: Table, current: Row) => ({ type, table, current });
const deletedRow = <Change extends RowChangeType & "Deleted", Table extends TableName, Row extends Option.Some>(type: Change, table: Table, deleted: Row) => ({ type, table, deleted });
const modifiedRow = <Change extends RowChangeType & "Updated", Table extends TableName, Row extends Option.Some>(type: Change, table: Table, current: Row, previous: Row) => ({ type, table, current, previous });
...
const rowChangeFactory = {
    ...
    actionItem_update: (current: ActionItem, previous: ActionItem) => modifiedRow("Updated", "ActionItem", current, previous)
}
...
export function createStorageService(): StorageService {
    ...
    db.on("changes", (changes: any) => {
        const result = changes.map((change: any) => {
            ...
            const changeType: RowChangeType | undefined = change.type === 1 ? "Inserted" : change.type === 2 ? "Updated" : change.type === 3 ? "Deleted" : undefined;
            ...
            switch (table) {
                case "ActionItem": {
                    switch (changeType) {
                        case "Inserted": return rowChangeFactory.actionItem_insert(current);
                        case "Updated": return rowChangeFactory.actionItem_update(current, previous);
                        case "Deleted": return rowChangeFactory.actionItem_delete(previous);
                    }
                }
                default:
